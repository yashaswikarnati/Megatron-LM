...
def configure_grad_sync(args, mimo_model: MimoModel, topology: HeteroTopology) -> None:
    ...
    def finalize_grads_func(_model_list, num_tokens, force_all_reduce=False, **_kwargs):
        ...
        with timeline_event("hetero.finalize.token_count_reduce"):
        ...
        with timeline_event("hetero.finalize.token_count_item"):
            # .item() is a host-blocking sync; blocks until prior CUDA work
            # on the default stream completes.
            global_num_tokens_value = global_num_tokens.item()

        if mimo_model.language_model is not None:
            debug_rank("finalizing language grads")
            with timeline_event("hetero.finalize.language_finalize_model_grads"):
        ...
        if lang_scale != 0.0 and mimo_model.language_model is not None:
            ...
            with timeline_event("hetero.finalize.language_scale_gradients"):
        ...
        for submodule in mimo_model.modality_submodules.values():
            ...
            with timeline_event("hetero.finalize.vision_finalize_model_grads"):
                finalize_model_grads(
                    [submodule],
                    num_tokens=None,
            ...
            if vision_scale != 0.0:
                ...
                with timeline_event("hetero.finalize.vision_scale_gradients"):
