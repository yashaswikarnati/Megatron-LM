...
RUN_NAME="mimo-jitter-gbs768-34n-cw-PG${OVERLAP_PARAM_GATHER}-GR${OVERLAP_GRAD_REDUCE}-NDOI${NUM_DIST_OPT_INSTANCES}-B${DDP_NUM_BUCKETS}-NAN${CHECK_FOR_NAN_IN_GRAD}-${_NCCL_PROTO_TAG}-NVLS${_NCCL_NVLS_TAG}-CDMC${_CDMC_TAG}-GC${_GC_TAG}-GCI${_GCINT_TAG}-DS${_DS_TAG}"
