#!/bin/bash
# cw-dfw variant of sbatch_hetero_jitter_gbs768_68n.sh — timeline-only 68n run
# for cross-cluster jitter comparison vs nb-hel 68n nsys (job 297782).
# 68 nodes (4n encoder DP=32 + 64n LLM TP=2 DP=256 EP=8), GBS=768, 100 iters.
...
#SBATCH -J mimo-jitter-gbs768-68n-cw
#SBATCH --exclusive
...
DDP_NUM_BUCKETS=${DDP_NUM_BUCKETS:-4}
...
HETERO_DISABLE_AUTO_GC="${HETERO_DISABLE_AUTO_GC:-1}"
HETERO_GC_INTERVAL="${HETERO_GC_INTERVAL:-30}"
export HETERO_DISABLE_AUTO_GC HETERO_GC_INTERVAL
_GC_TAG="${HETERO_DISABLE_AUTO_GC}"
...
RUN_NAME="mimo-jitter-gbs768-68n-cw-PG${OVERLAP_PARAM_GATHER}-GR${OVERLAP_GRAD_REDUCE}-NDOI${NUM_DIST_OPT_INSTANCES}-B${DDP_NUM_BUCKETS}-NAN${CHECK_FOR_NAN_IN_GRAD}-${_NCCL_PROTO_TAG}-NVLS${_NCCL_NVLS_TAG}-CDMC${_CDMC_TAG}-GC${_GC_TAG}-GCI${_GCINT_TAG}-DS${_DS_TAG}-W${NUM_WORKERS:-2}-PB${PACKING_BUFFER_SIZE:-4}"
RUN_DIR="${SCRATCH_ROOT}/runs/${RUN_NAME}/${SLURM_JOB_ID:-local}"

# ---- topology: TP=2 EP=8 LLM (64 nodes, DP=256) + TP=1 DP=32 encoder lane (4 nodes)
...
NUM_MICROBATCHES=$(( GLOBAL_BATCH_SIZE / (MICRO_BATCH_SIZE * LLM_DP) ))   # = 3 (GBS=768 / DP=256)
TRAIN_ITERS=100
LOG_INTERVAL=1
...
# LL128 is the validated default on cw-dfw at 68n (job 12251160): p99 -20%,
...
echo "=== hetero JITTER GBS=768 68n cw-dfw (${TRAIN_ITERS} iters, PG=${OVERLAP_PARAM_GATHER} GR=${OVERLAP_GRAD_REDUCE} FLB=${MOE_ROUTER_FORCE_LOAD_BALANCING} NDOI=${NUM_DIST_OPT_INSTANCES} BUCKETS=${DDP_NUM_BUCKETS} NAN=${CHECK_FOR_NAN_IN_GRAD} NCCL_PROTO=${NCCL_PROTO} NCCL_NVLS=${NCCL_NVLS_ENABLE}) ==="
