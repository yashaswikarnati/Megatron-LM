...
# Forces virtual llm_dp=384 so lanes_per_encoder=12 — exact encoder-side
# fan-out used in the 100n EP=8 production scaling run. No LLM, no bridge,
...
#SBATCH -J routed-enc-only-4n-lanes12
#SBATCH --exclusive
...
RUN_NAME="routed-enc-only-4n-lanes12"
RUN_DIR="${SCRATCH_ROOT}/runs/${RUN_NAME}/${SLURM_JOB_ID:-local}"

ENCODER_DP=32
LLM_DP=384               # virtual; gives lanes_per_encoder=12 (matches 100n EP=8 production fan-out)
N_STEPS=${N_STEPS:-200}
...
echo "=== routed-encoder-only 4 nodes 32 GPUs (TP=1 DP=${ENCODER_DP}, virtual llm_dp=${LLM_DP}, lanes=12 — mirrors 100n encoder) ==="
