#!/bin/bash
# Short hetero-MIMO parity test for num_distributed_optimizer_instances on LLM.
# 9 nodes (1n encoder DP=8 + 8n LLM TP=2 DP=32 EP=16), GBS=192, 25 iters.
# Run twice — once with NUM_DIST_OPT_INSTANCES=1 (baseline), once with N=2 —
# and compare per-iter loss + grad norm. RUN_NAME is suffixed by the N value
# so the two output dirs do not collide.
...
#SBATCH -J mimo-parity-gbs192-numinst
#SBATCH --exclusive
...
RUN_NAME="mimo-parity-gbs192-n${NUM_DIST_OPT_INSTANCES}"
...
echo "=== hetero parity GBS=192 (${TRAIN_ITERS} iters, NUM_DIST_OPT_INSTANCES=${NUM_DIST_OPT_INSTANCES}) ==="
