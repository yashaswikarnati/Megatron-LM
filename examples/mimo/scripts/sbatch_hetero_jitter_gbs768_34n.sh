#!/bin/bash
# Jitter-investigation run at 34 nodes / GBS=768.
# 34 nodes (2n encoder DP=16 + 32n LLM TP=2 DP=128 EP=8), GBS=768, 100 iters.
# Encoder over-provisioned (2 enc nodes vs 1 at 33n) so encoder is definitely not
# the bottleneck. Same EP=8 LLM config exhibits the same mb0-fwd / mb_last-bwd
# absorption pattern at smaller wall cost.
#
# All 272 ranks traced with timeline JSONL. New instrumentation captures:
#   - ddp.start_param_sync / ddp.start_grad_sync (per-bucket dispatch time)
#   - moe.dtoh_sync (host-blocking sync inside MoE token dispatcher)
#   - moe.a2a_dispatch_tokens / moe.a2a_dispatch_probs / moe.a2a_combine
#     (EP=8 NCCL all-to-alls inside each MoE layer)
#
# Env knobs:
#   OVERLAP_PARAM_GATHER=1 (default) → --overlap-param-gather (Run A)
#   OVERLAP_PARAM_GATHER=0           → --no-overlap-param-gather (Run B controlled)
...
#SBATCH -J mimo-jitter-gbs768-34n
#SBATCH --exclusive
