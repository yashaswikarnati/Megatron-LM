#!/bin/bash
# Hetero MIMO at EP=8 (vs the parity sbatch's EP=16) to measure iter-rate
# impact of smaller expert-parallel groups. Saves a ckpt every 1000 iters.
# 9 nodes (1n encoder DP=8 + 8n LLM TP=2 DP=32 EP=8 EDP=8). GBS=192,
...
RUN_NAME="mimo-ep8-5k"
RUN_DIR="${SCRATCH_ROOT}/runs/${RUN_NAME}/${SLURM_JOB_ID:-local}"

# ---- topology: 1n encoder (DP=8) + 8n LLM (TP=2 DP=32 EP=8 EDP=8) ------------
ENCODER_TP=1; ENCODER_CP=1; ENCODER_PP=1; ENCODER_DP=8;  ENCODER_EP=1
LLM_TP=2;     LLM_CP=1;     LLM_PP=1;     LLM_DP=32;    LLM_EP=8;    LLM_EXPT_TP=1
LLM_ONLY=0
...
srun --kill-on-bad-exit=1 \
  ...
  bash -lc 'set -euo pipefail; cd "${REPO_ROOT}";
    if [ -n "${USE_HETERO_ENERGON_OVERRIDE:-}" ]; then
      ...
      SENT="/tmp/.energon_installed_${SLURM_JOB_ID}"
      if [ "${SLURM_LOCALID}" -eq 0 ]; then
        echo "[hetero_sbatch] swapping energon: $ESRC -> $EDST"
        rm -rf "$EDST"; cp -r "$ESRC" "$EDST"; touch "$SENT"
      else
