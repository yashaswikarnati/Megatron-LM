#!/bin/bash
# 3n hetero MIMO GBS=32 parity run with a py-spy sidecar attached for stall
# capture. Clone of sbatch_hetero_parity_gbs32.sh; the ONLY differences are:
#   - PYSPY_SIDECAR=1 by default
#   - srun command invokes run_hetero_pyspy_wrap.sh instead of the runner
# The wrapper spawns the sidecar on LOCAL_RANK=0 of each node, then exec's
# the unmodified run_hetero_nemotron_54l_hel_train.sh.
#
# Sidecar output: ${RUN_DIR}/pyspy/<node>/pid<PID>_<ts>.txt
#
# Knobs (override at submit time):
#   PYSPY_SIDECAR        default 1 (set 0 to silently no-op the sidecar)
#   PYSPY_INTERVAL_SEC   default 0.5
#   PYSPY_DURATION_SEC   default 3000  (50 min — covers full job)
#   PYSPY_PROC_MATCH     default train_hetero.py
#
# Example submit:
#   PYSPY_INTERVAL_SEC=0.25 sbatch examples/mimo/scripts/sbatch_hetero_parity_gbs32_pyspy.sh
...
#SBATCH -J mimo-parity-gbs32-pyspy
#SBATCH --exclusive
...
LLM_EP=${LLM_EP:-16}
DISABLE_THP=${DISABLE_THP:-0}
RUN_NAME="mimo-parity-gbs32-pyspy-EP${LLM_EP}-PG${OVERLAP_PARAM_GATHER}-GR${OVERLAP_GRAD_REDUCE}-FLB${MOE_ROUTER_FORCE_LOAD_BALANCING}-THP${DISABLE_THP}"
RUN_DIR="${SCRATCH_ROOT}/runs/${RUN_NAME}/${SLURM_JOB_ID:-local}"

# ---- topology: TP=2 LLM + TP=1 DP=8 encoder lane.  LLM_EP overridable. ----
ENCODER_TP=1; ENCODER_CP=1; ENCODER_PP=1; ENCODER_DP=8; ENCODER_EP=1
LLM_TP=2;     LLM_CP=1;     LLM_PP=1;     LLM_DP=8;              LLM_EXPT_TP=1
LLM_ONLY=0
...
# ---- py-spy sidecar knobs ----
export PYSPY_SIDECAR="${PYSPY_SIDECAR:-1}"
export PYSPY_INTERVAL_SEC="${PYSPY_INTERVAL_SEC:-0.5}"
export PYSPY_DURATION_SEC="${PYSPY_DURATION_SEC:-3000}"
export PYSPY_PROC_MATCH="${PYSPY_PROC_MATCH:-train_hetero.py}"
export PYSPY_OUT_DIR="${RUN_DIR}/pyspy"
...
echo "=== hetero parity GBS=32 + py-spy (${TRAIN_ITERS} iters, PG=${OVERLAP_PARAM_GATHER} GR=${OVERLAP_GRAD_REDUCE}) ==="
...
echo "pyspy: sidecar=${PYSPY_SIDECAR} interval=${PYSPY_INTERVAL_SEC}s dur=${PYSPY_DURATION_SEC}s out=${PYSPY_OUT_DIR}"
echo "=================================================="
...
srun --kill-on-bad-exit=1 \
  ...
  bash -lc 'set -euo pipefail; cd "${REPO_ROOT}";
    if [ -n "${HETERO_SKIP_UV:-}" ]; then export PYTHONPATH="${REPO_ROOT}:${PYTHONPATH:-}"; exec bash examples/mimo/scripts/run_hetero_pyspy_wrap.sh "$@"; else exec uv run --no-sync bash examples/mimo/scripts/run_hetero_pyspy_wrap.sh "$@"; fi
  ' \
