#!/bin/bash
# Nsys profile run at 34 nodes / GBS=768.
# Matches Run A config (PG=1, GR=1) so nsys data is directly comparable to the
# JSONL timeline data we already have.
#
# Profile window: iters 30-40 (11 iters). Should include a mix of slow and fast
# iters based on the steady-state distribution (median 5.7s, p90 6.4s).
# Profiled ranks: 16..23 (one EP group, all 8 expert positions × TP=2 pairs).
...
RUN_NAME="mimo-nsys-gbs768-34n-PG${OVERLAP_PARAM_GATHER}-GR${OVERLAP_GRAD_REDUCE}"
...
TRAIN_ITERS=50  # warmup + 11-iter profile window (30-40) + small buffer
LOG_INTERVAL=1
...
# nsys profile knobs — the inner run script (run_hetero_nemotron_54l_hel_train.sh)
# wraps python with `nsys profile` ONLY for ranks in NSYS_RANKS.  The in-code
# cudaProfilerStart/Stop (loop.py) bounds the actual capture window to iters
# [profile_step_start, profile_step_end].
NSYS_RANKS="${NSYS_RANKS:-16,17,18,19,20,21,22,23}"
NSYS_OUT_DIR="${RUN_DIR}/nsys"
export NSYS_RANKS NSYS_OUT_DIR

# Timeline JSONL still ON (gives us the host-side view to overlay with nsys).
TIMELINE=${TIMELINE:-1}
...
TRAIN_LAUNCH_ARGS=(
  ...
  # Megatron profile flags — loop.py uses these to fire cudaProfilerStart/Stop.
  --profile
  --profile-step-start 30
  --profile-step-end 40
  --profile-ranks 16 17 18 19 20 21 22 23
  --nvtx-ranges
  --record-shapes
)
...
echo "=== hetero NSYS GBS=768 34n (${TRAIN_ITERS} iters, profile 30-40 on ranks ${NSYS_RANKS}, PG=${OVERLAP_PARAM_GATHER} GR=${OVERLAP_GRAD_REDUCE}) ==="
