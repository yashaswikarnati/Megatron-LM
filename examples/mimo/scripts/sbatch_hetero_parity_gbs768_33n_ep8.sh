#!/bin/bash
# Scaling-study run at 33 nodes / GBS=768 with LLM_EP=8 (vs EP=16 in the
# baseline). 33 nodes (1n encoder DP=8 + 32n LLM TP=2 DP=128 EP=8),
# expt_dp=32, GBS=768, 25 iters, 1 h wall. Probe for EP=8 viability before
# scaling to 100n (where 100n/EP=8 had expt_dp=96 and deadlocked iter 1).
...
#SBATCH -J mimo-scaling-gbs768-33n-ep8
#SBATCH --exclusive
...
RUN_NAME="mimo-scaling-gbs768-33n-ep8"
...
echo "=== hetero scaling GBS=768 33n EP=8 expt_dp=32 (${TRAIN_ITERS} iters, 1 h wall, force-LB=${MOE_ROUTER_FORCE_LOAD_BALANCING}) ==="
