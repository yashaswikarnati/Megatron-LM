...
RUN_NAME="mimo-nsys-gbs32-EP${LLM_EP}-PG${OVERLAP_PARAM_GATHER}-GR${OVERLAP_GRAD_REDUCE}-FLB${MOE_ROUTER_FORCE_LOAD_BALANCING}-THP${DISABLE_THP}"
RUN_DIR="${SCRATCH_ROOT}/runs/${RUN_NAME}/${SLURM_JOB_ID:-local}"

# ---- topology: TP=2 LLM + TP=1 DP=8 encoder lane.  LLM_EP overridable (default 8 to match 34n). ----
