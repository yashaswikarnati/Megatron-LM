#!/bin/bash
# Scaling-study run at 100 nodes / GBS=768 with forced MoE load balancing.
# 100 nodes (4n encoder DP=32 + 96n LLM TP=2 DP=384 EP=8), GBS=768, 25 iters.
# Bumped NCCL timeout to 25 min via HETERO_DIST_TIMEOUT_MIN to ride out the
# slow first-iter collective at expt_dp=96 that hit the default 10 min
# watchdog in the prior 100n run.
