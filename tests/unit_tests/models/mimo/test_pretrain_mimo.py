# Copyright (c) 2026, NVIDIA CORPORATION. All rights reserved.

"""Focused tests for the heterogeneous MIMO training entrypoint."""

import argparse
from pathlib import Path
from types import SimpleNamespace

import pytest

from examples.mimo import pretrain_mimo
from examples.mimo.model_providers.radio_encoder import RADIO_ENCODER_MODULE_NAME
from megatron.core.enums import ModelType
from megatron.core.models.mimo.config.role import MIMO_LANGUAGE_MODULE_KEY


class _LogicalDp(int):
    """Non-interned integer used to assert identity across validation."""


def test_parse_and_validate_orders_grid_before_stock_and_checks_validated_optimizer(monkeypatch):
    logical_dp = _LogicalDp(2)
    args = SimpleNamespace(
        use_distributed_optimizer=True,
        world_size=8,
        llm_dp=logical_dp,
        llm_tp=2,
        padded_vocab_size=None,
        vocab_size=511,
        make_vocab_size_divisible_by=128,
    )
    calls = []
    reject_after_validation = False

    parser = pretrain_mimo.extra_args_provider(argparse.ArgumentParser())
    local_args = parser.parse_args([])
    assert local_args.dataset_provider == "mock"
    assert local_args.image_token_id == 511
    assert local_args.image_seq_length is None

    def fake_parse_args(extra_args_provider):
        calls.append("parse")
        assert extra_args_provider is pretrain_mimo.extra_args_provider
        return args

    def fake_validate_grid(received, world_size):
        calls.append("grid")
        assert received is args
        assert world_size == 8

    def fake_validate_args(received, defaults, data_parallel_size_override=None):
        calls.append("stock")
        assert received is args
        assert defaults == {"dataloader_type": "external"}
        assert data_parallel_size_override is logical_dp
        if reject_after_validation:
            received.use_distributed_optimizer = False
        return received

    def fake_calculate(vocab_size, make_divisible_by, tp_size, *, logging_enabled):
        calls.append("vocab")
        assert (vocab_size, make_divisible_by, tp_size, logging_enabled) == (511, 128, 2, False)
        return 512

    monkeypatch.setattr(pretrain_mimo, "parse_args", fake_parse_args)
    monkeypatch.setattr(pretrain_mimo, "validate_hetero_grid_args", fake_validate_grid)
    monkeypatch.setattr(pretrain_mimo, "validate_args", fake_validate_args)
    monkeypatch.setattr(pretrain_mimo, "calculate_padded_vocab_size", fake_calculate)

    assert pretrain_mimo._parse_and_validate() is args
    assert args.padded_vocab_size == 512
    assert calls == ["parse", "grid", "stock", "vocab"]

    reject_after_validation = True
    args.use_distributed_optimizer = True
    args.padded_vocab_size = None
    calls.clear()
    with pytest.raises(ValueError, match="--use-distributed-optimizer"):
        pretrain_mimo._parse_and_validate()
    assert calls == ["parse", "grid", "stock"]


def test_main_wires_iterators_communicator_carrier_and_cleans_up_on_failure(mocker):
    args = SimpleNamespace(world_size=8)
    encoder_grid = object()
    language_grid = object()
    process_groups = object()
    communicator = object()
    language_config = object()
    valid_loader = iter(["valid"])
    loaders = (["train"], valid_loader, None)
    topology = SimpleNamespace(
        grids={RADIO_ENCODER_MODULE_NAME: encoder_grid, MIMO_LANGUAGE_MODULE_KEY: language_grid},
        pg_collection=process_groups,
        destroy=mocker.Mock(),
    )

    mocker.patch.object(pretrain_mimo, "_parse_and_validate", return_value=args)
    mocker.patch.object(pretrain_mimo, "set_global_variables")
    mocker.patch.object(pretrain_mimo, "initialize_distributed")
    mocker.patch.object(pretrain_mimo, "build_module_grid_specs", return_value=object())
    mocker.patch.object(pretrain_mimo, "create_topology", return_value=topology)
    mocker.patch.object(
        pretrain_mimo,
        "language_model_spec",
        return_value=SimpleNamespace(params={"config": language_config}),
    )

    def fake_communicator(grids, dependencies, config, *, dim_mapping, module_output_ndim):
        assert grids is topology.grids
        assert dependencies == {
            RADIO_ENCODER_MODULE_NAME: [MIMO_LANGUAGE_MODULE_KEY],
            MIMO_LANGUAGE_MODULE_KEY: [],
        }
        assert config is language_config
        assert dim_mapping == {"s": 0, "h": 2, "b": 1}
        assert module_output_ndim == {RADIO_ENCODER_MODULE_NAME: 2}
        return communicator

    mocker.patch.object(pretrain_mimo, "MultiModulePipelineCommunicator", fake_communicator)
    mocker.patch.object(pretrain_mimo, "build_train_valid_test_data_loaders", return_value=loaders)
    mocker.patch.object(pretrain_mimo, "pretrain_cfg_container_from_args", return_value="cfg")

    def fake_pretrain(*call_args, **kwargs):
        _, provider, model_type, forward_step = call_args
        iterators = provider(None)
        assert all(iterator is None or iter(iterator) is iterator for iterator in iterators)
        assert iterators[1] is valid_loader
        assert provider.is_distributed is True
        assert model_type is ModelType.encoder_or_decoder
        assert forward_step is pretrain_mimo.mimo_forward_step
        assert kwargs["p2p_communicator"] is communicator
        assert kwargs["pg_collection"] is process_groups
        raise RuntimeError("training failed")

    mocker.patch.object(pretrain_mimo, "pretrain", fake_pretrain)
    shutdown = mocker.patch.object(pretrain_mimo, "shutdown_distributed")

    with pytest.raises(RuntimeError, match="training failed"):
        pretrain_mimo.main()

    topology.destroy.assert_called_once_with()
    shutdown.assert_called_once_with()


def test_main_shuts_down_after_partial_distributed_initialization(mocker):
    mocker.patch.object(
        pretrain_mimo, "_parse_and_validate", return_value=SimpleNamespace(world_size=8)
    )
    mocker.patch.object(pretrain_mimo, "set_global_variables")
    mocker.patch.object(
        pretrain_mimo,
        "initialize_distributed",
        side_effect=RuntimeError("partial initialization failed"),
    )
    shutdown = mocker.patch.object(pretrain_mimo, "shutdown_distributed")

    with pytest.raises(RuntimeError, match="partial initialization failed"):
        pretrain_mimo.main()

    shutdown.assert_called_once_with()


def test_launcher_always_configures_unique_per_rank_logs():
    launcher = (
        Path(__file__).parents[4] / "examples/mimo/scripts/run_hetero_nemotron_20l_mock_train.sh"
    ).read_text()

    assert 'TORCHRUN_LOG_DIR=${TORCHRUN_LOG_DIR:-"${PWD}/logs/torchrun-' in launcher
    assert 'mkdir -p "${TORCHRUN_LOG_DIR}"' in launcher
    assert '--log-dir "${TORCHRUN_LOG_DIR}"' in launcher
    assert "--redirects 3" in launcher
    assert "--tee 0:3" in launcher
    assert 'if [[ -n "${TORCHRUN_LOG_DIR:-}" ]]' not in launcher
    assert "--moe-flex-dispatcher-backend" not in launcher
