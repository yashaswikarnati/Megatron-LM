...
The ``item4`` (process-group resolution) and ``item1`` (cross-rank ``count_zeros``)
tests are REAL distributed tests: they initialize the process group, build a real
``HyperCommGrid`` spanning all 8 ranks, register the expert layout via the
``register_layout(...)->GridLayout`` handle API, create the base + expert-layout
process groups, and assert actual group membership / actual collective results.
They run on the standard 1-node x 8-GPU unit-test lane and are intentionally NOT
skip-gated on world size: the lane always launches 8 ranks, so the tests assert
``world_size == 8`` and fail loudly if that invariant is ever violated (a plain
``pytest.skip`` would silently hide a regression on the 8-GPU lane).
...
def _build_dense_grid(tp=1, cp=1, pp=1, dp=1, ep=1, expt_dp=1, register_expert=False):
    ...
    The base layout carries the dense ``tp/cp/pp/dp`` factorization (plus degenerate
    ``ep``/``expt_dp`` placeholders for backward compatibility). When ``register_expert``
    is set, a separate ``"expert"`` layout (``expt_tp/ep/expt_dp/pp``) is registered via
    the new ``register_layout(...)->GridLayout`` handle API with ``pp`` declared a shared
    dim, and its groups are created through the handle, exercising the heterogeneous path.
    """
    grid = HyperCommGrid(
    ...
    if register_expert:
        ...
        # base layout: ``register_layout`` enforces that a shared dim enumerate to the SAME
        ...
        expert.create_pg(["expt_tp", "ep", "pp"])
        expert.create_pg(["expt_dp"])
    else:
        grid.create_pg(["ep"])
...
class TestGetPgCollectionForOptimizer:
    """item4: expert-layout group resolution + narrowed intra_dist_opt.
    ...
    def test_expert_layout_groups_and_dense_intra_dist_opt(self, dist_env):
        world_size = dist_env
        # Intentionally an assert, not a skip: the unit-test lane is always 8 GPUs,
        # so this test MUST run there. Failing loudly on a wrong world size beats a
        # silent skip that would hide a regression on the 8-GPU lane.
        assert world_size == 8, f"This real distributed test requires 8 GPUs, got {world_size}"
        ...
        try:
            ...
            # Dense groups come from the base layout.
            ...
            # Expert groups come from the registered "expert" layout handle, NOT the
            # base dims.
            expert = grid.get_layout(EXPERT_LAYOUT)
            assert pg.tp_ep_pp is expert.get_pg(["expt_tp", "ep", "pp"])
            assert pg.expt_dp is expert.get_pg(["expt_dp"])
            assert pg.intra_expt_dp is pg.expt_dp

            # pp is a SHARED dim of the expert layout: the expert [expt_tp, ep, pp]
            # group's pp membership must agree with the base pp group, and the layout
            # reuses the base pp group object for a pure-pp request.
            assert expert.get_pg(["pp"]) is grid.get_pg("pp")

            # The expert tp_ep_pp partition differs from the base [tp,pp] (mp) partition:
            # it additionally folds in the ep axis, so a heterogeneous expert layout is
            # genuinely honored rather than silently aliased onto the dense groups.
            assert _ranks(pg.tp_ep_pp) != _ranks(pg.mp)
        finally:
            grid.destroy()

    def test_falls_back_to_base_layout_without_expert_layout(self, dist_env):
        world_size = dist_env
        assert world_size == 8, f"This real distributed test requires 8 GPUs, got {world_size}"
        ...
        try:
            pg = _get_pg_collection_for_optimizer(grid)
            # No expert layout -> expert groups resolve against the base layout.
...
class TestCountZeros:
    """item1: count_zeros must be consistent across ranks in non-colocated mode.
...
class TestShardedKeyCollision:
    """item2: add_prefix_for_sharding disambiguates colliding inner optimizer keys.
